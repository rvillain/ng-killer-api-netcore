...
namespace WebPush
{
    public class WebPushClient
    {
        ...
        private ILogger<WebPushClient> _logger;
        ...
        public async Task SendNotificationAsync(PushSubscription subscription, string payload = null, Dictionary<string, object> options = null)
        {
            ...
            Console.WriteLine(@"uri: " + request.RequestUri.AbsoluteUri);
            ...
            if (response.StatusCode != System.Net.HttpStatusCode.Created) //201
            {
                ...
                _logger.LogError(request.Headers.Authorization.Scheme + " " + request.Headers.Authorization.Parameter );
                Console.WriteLine(@"status code: " + response.StatusCode.ToString());
                Console.WriteLine(@"content: " + response.Content.ToString());
                throw new WebPushException(@"Received unexpected response code " + response.StatusCode.ToString(), response.StatusCode, response.Headers, subscription);
            }
